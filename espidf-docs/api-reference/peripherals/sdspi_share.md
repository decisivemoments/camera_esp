...
1. Use an oscilloscope to see the clock and compare the data line to the clock.
	...
	* If you see the data line unstable before the latch edge of the clock, it means the load of the data line is too large.You may also observe the corresponding phenomenon that data delayed largely from the launching edge of the clock with logic analyzers. But it is not as obvious as with an oscilloscope.
