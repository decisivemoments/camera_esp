...
!Diagram of ESP-WIFI-MESH Network Architecture
...
!Diagram of ESP-WIFI-MESH Tree Topology
...
!Diagram of ESP-WIFI-MESH Node Types
...
!Diagram of the Effects of RSSI Thresholding
...
!Diagram of Preferred Parent Node Selection
...
Users may also define their own algorithm for selecting a preferred parent node, or force a node to only connect with a specific parent node (see the Mesh Manual Networking Example ).
...
!Diagram of ESP-WIFI-MESH Routing Tables Example
...
Users can call `esp_mesh_get_routing_table()` to obtain a node's routing table, or `esp_mesh_get_routing_table_size()` to obtain the size of a node's routing table. `esp_mesh_get_subnet_nodes_list()` can be used to obtain the corresponding subtable of a specific child node. Likewise `esp_mesh_get_subnet_nodes_num()` can be used to obtain the size of the subtable.
...
Before the ESP-WIFI-MESH network building process can begin, certain parts of the configuration must be uniform across each node in the network (see `mesh_cfg_t`). Each node must be configured with **the same Mesh Network ID, router configuration, and softAP configuration**.
...
!Diagram of ESP-WIFI-MESH Network Building Process
...
!Diagram of Root Node Election Process Example
...
The minimum number of iterations for the election process can be configured using `esp_mesh_set_attempts()`. Users should adjust the number of iterations based on the number of nodes within the network (i.e., the larger the network the larger number of scan iterations required).
...
!Diagram of Root Node Designation Process Example
...
When designating a root node, the root node should call `esp_mesh_set_parent()` in order to directly connect with the router. Likewise, all other nodes should call `esp_mesh_fix_root()` to forgo the election process.
...
!Diagram of Asynchronous Power On Example
...
Differences in parent node selection caused by asynchronous power-on are autonomously corrected for to some extent in ESP-WIFI-MESH (see Parent Node Switching)
...
!Diagram of Self Healing From Intermediate Parent Node Failure
...
!Diagram of ESP-WIFI-MESH Packet
...
Due to Parent Node Switching, packet loss may occur during upstream transmissions.
...
!Diagram of ESP-WIFI-MESH Bidirectional Data Stream
...
**Preset Router BSSID:** An ESP-WIFI-MESH network can have a preset router by setting the `bssid` field of the `mesh_router_t` structure to the
