...
* code in RTC memory for use by the ULP coprocessor.
...
The linker script template currently used is esp_system/ld/esp32s3/sections.ld.in; the generated output script `sections.ld` is put under its build directory.